...
export default function PhoneFrame({ children, theme }: PhoneFrameProps) {
  ...
  return (
    <div className="flex justify-center items-center py-6 px-4 bg-zinc-100 min-h-screen text-slate-900 select-none font-sans transition-colors duration-300">
      {/* Decorative Outer Ambient Glow */}
      <div className="relative w-full max-w-[420px] aspect-[9/19] rounded-[52px] bg-neutral-900 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.5),0_0_40px_rgba(140,106,77,0.15)] border-4 border-neutral-800 flex flex-col overflow-hidden ring-12 ring-zinc-950/20">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 h-7 w-36 bg-black rounded-b-2xl z-50 flex items-center justify-center gap-1.5 px-3">
          {/* Camera lens */}
          <div className="w-2.5 h-2.5 rounded-full bg-zinc-900 border border-zinc-800/80"></div>
          {/* Speaker slit */}
        ...
        <div className={`h-11 pt-7 px-6 flex justify-between items-center text-xs font-semibold z-40 relative ${
          theme === 'dark' ? 'bg-[#3B2E2B] text-amber-100' : 'bg-[#FEEBC5] text-[#3B2E2B]'
        }`}>
          <div>{currentTime}</div>
        ...
        <div className={`h-6 flex justify-center items-center relative z-40 ${
          theme === 'dark' ? 'bg-[#332724]' : 'bg-[#FAECE1]'
        }`}>
          <div className="w-32 h-1 bg-neutral-400 rounded-full opacity-40"></div>
        </div>
